...
────────────────────────────────────────
Gemini 2.5 Flash API (무료 티어)로 24시간 이내 기사를 수집·요약하고
...
- Google Fonts CDN 임포트 포함 / 완전한 HTML 문서로 출력"""


# ─── 2. Gemini API 호출 (표준 라이브러리만 사용) ─
def generate_brief(today: str, yesterday: str) -> str:
    ...
    payload = json.dumps({
        ...
        "contents": [{
            "parts": [{"text": build_prompt(today, yesterday)}]
        }],
        "generationConfig": {
...
def extract_html(raw: str) -> str:
    ...
    return m.group(1).strip() if m else (
        ...
        "background:#fafaf8;padding:20px;max-width:780px;margin:auto'>"
        f"{raw}</body></html>"
    )
