...
def send_newsletter():
    ...
    # 4. 모델 리스트 및 주소 조립 (기호 오류 방지를 위해 쪼개서 합침)
    models = ["gemini-2.0-flash", "gemini-1.5-flash"]
    raw_text = None

    print(f"🚀 {today_str} 뉴스레터 생성 시작 (24시간 이내 소식 20건)...")

    # 주소 체계 (마크다운 링크 방지를 위해 수동 결합)
    base_host = "[https://generativelanguage.googleapis.com](https://generativelanguage.googleapis.com)"
    api_version = "/v1beta/models/"

    for model_name in models:
        if raw_text: break
        
        # 주소에 대괄호([])가 절대 섞이지 않도록 함
        full_url = base_host + api_version + model_name + ":generateContent?key=" + api_key
        ...
        for i in range(3):
            try:
                ...
                print(f"⚠️ {model_name} 연결 에러 발생 (주소 형식을 확인하세요)")
                break
